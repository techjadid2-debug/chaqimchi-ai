#!/usr/bin/env bash
# ENES Monitoring rasmiy saytidan birinchi Sotqin o'rnatishi uchun bootstrap.
# Release SHA-256 cloud tomonidan beriladi; keyingi relizlar signed updater bilan o'tadi.
set -euo pipefail

release_url="__RELEASE_URL__"
release_sha256="__RELEASE_SHA256__"
cloud_url="__CLOUD_URL__"
pairing_code=""

while [[ $# -gt 0 ]]; do
  case "$1" in
    --cloud) cloud_url="$2"; shift 2 ;;
    --code) pairing_code="$2"; shift 2 ;;
    *) echo "Noma'lum parametr: $1" >&2; exit 2 ;;
  esac
done

if [[ "$(id -u)" -ne 0 ]]; then
  echo "Installer sudo/root bilan ishga tushishi kerak" >&2
  exit 1
fi
if [[ ! "$pairing_code" =~ ^[A-Fa-f0-9]{6}$ ]]; then
  echo "--code bilan 6 xonali pairing kod bering" >&2
  exit 2
fi
if [[ "$release_url" == __* || "$release_sha256" == __* ]]; then
  echo "Sotqin release hali admin tomonidan nashr qilinmagan" >&2
  exit 3
fi

workdir="$(mktemp -d /tmp/enes-sotqin.XXXXXX)"
cleanup() { rm -rf "$workdir"; }
trap cleanup EXIT
archive="$workdir/release.tar.gz"
curl --fail --location --proto '=https' --tlsv1.2 "$release_url" -o "$archive"
echo "$release_sha256  $archive" | sha256sum --check --status
mkdir -p "$workdir/release"
tar -xzf "$archive" -C "$workdir/release"
installer="$(find "$workdir/release" -path '*/scripts/install_sotqin.sh' -type f -print -quit)"
if [[ -z "$installer" ]]; then
  echo "Release ichida Sotqin installer topilmadi" >&2
  exit 4
fi
bash "$installer"
/opt/enes/venv/bin/python /opt/enes/current/scripts/pair_sotqin.py \
  --cloud "$cloud_url" --code "${pairing_code^^}"
systemctl restart enes-sotqin
systemctl restart enes-retail
systemctl --no-pager --full status enes-sotqin enes-retail >/dev/null
echo
echo "── Tekshiruv ro'yxati ─────────────────────────────────────────────"
# Xizmatlar ko'tarilishi va birinchi config kelishi uchun biroz vaqt kerak.
sleep 10
# Tekshiruv yiqilsa ham o'rnatish bekor qilinmaydi: qurilma o'rnatilgan,
# faqat kamera yoki chiziq hali sozlanmagan bo'lishi mumkin.
/opt/enes/venv/bin/python /opt/enes/current/scripts/sotqin_preflight.py || true
echo
echo "Sotqin o'rnatildi. Yuqoridagi ro'yxatda ✗ bo'lmasa obyektni topshirish mumkin."
