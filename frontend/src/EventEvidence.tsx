...
function Evidence({ item, kind, siteId }: { item: Event; kind: "owner" | "admin"; siteId?: string }) {
...
export function EventEvidence({ kind, siteId, sites }: { kind: "owner" | "admin"; siteId?: string; sites?: Array<{id:string;name:string}> }) {
  ...
  return <><PageHeader title="Hodisalar va dalillar" subtitle="Kadr yoki klip faqat u mavjud bo‘lgan eventda ochiladi." actions={kind === "admin" ? <select className="select" value={selected} onChange={event => setSelected(event.target.value)}><option value="">Barcha filiallar</option>{sites?.map(site => <option value={site.id} key={site.id}>{site.name}</option>)}</select> : <button className="btn" onClick={load}>Yangilash</button>}/>
    ...
    {error ? <div className="alert-strip alert-info"><Icon name="bell"/>{error}</div> : null}<Card>{events === null ? <div className="card-body"><Skeleton height={180}/></div> : events.length ? <div className="evidence-list">{events.map((item, index) => <Evidence key={item.id || item.event_id || index} item={item} kind={kind} siteId={kind === "owner" ? siteId : selected || item.site_id}/>)}</div> : <EmptyState icon="pulse" title="Hodisa yo‘q" detail="Qurilma AI hodisa yuborgach uning vaqti va dalili shu yerda ko‘rinadi."/>}</Card></>;
}
