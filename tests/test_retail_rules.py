...
def test_restricted_condition_does_not_alert_for_normal_zone() -> None:
    ...
    normal = EdgeEvent(event_type="zone_entered", camera_id="cam", metadata={"restricted": False})
...
def test_outside_schedule_covers_after_hours_without_a_second_window() -> None:
    ...
    assert (
        engine.evaluate(event, now=0.0, local_time=clock_time(2, 30)).event.severity == "critical"
    )
...
def test_suppress_rule_drops_the_event_entirely() -> None:
    engine = RuleEngine(
        [Rule(name="xodimlar-zonasi", event_type="person_detected", zone="xodimlar", suppress=True)]
    )
